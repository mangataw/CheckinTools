...
- 如果进入首页时，`div.tip_c` 包含“今日已签到”“今天已签到”等当日状态，或包含
  “今日第 N 个签到”排名文案，返回 `ALREADY_DONE`，不会再次调用签到接口。
- 页面中静态存在的 `#fx_checkin_menut`“签到成功”弹窗模板不能证明当天已签到，检查器会忽略
  这类未显示的通用模板。
- 如果尚未签到，则从 `fx_checkin()` 中提取同主机 HTTPS 签到链接并发送请求。
- 请求后再次读取首页并重新核对用户名。出现当日状态或排名文案、签到提示文本发生变化、
  积分文本发生变化，任一证据成立即返回 `SUCCESS`，并在结果摘要中列出确认依据。
- 提交后以上证据都不存在时返回 `FAILED`。
