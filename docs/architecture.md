...
当前核心只支持“每个账号每天一次”的幂等签到或领取任务。监控、下载、备份和多阶段流程不适用
现有 Checker、每日状态与通知语义。出现首个明确需求时，应根据它的调度、状态、产物和通知要求
设计最小扩展，不预先建立通用任务层。
...
- 账号凭据变化会生成新的状态指纹；Checker 仍须依靠站点证据避免重复签到。
- 单站点运行只隔离其他站点的账号和基础地址错误，通用网络与通知配置仍会整体校验。
- 青龙配置升级只追加缺失字段；删除或重命名字段需要提供明确的人工迁移说明。
- 通知失败不会撤销签到成功状态，也不会在当天后续运行中单独补发成功通知。
