...
订阅完成后，「定时任务」应为每个内置站点出现一个独立任务。
JavBus 和福利吧按青龙容器当地时间每天 `00:30` 和 `08:30` 运行。V2EX 在使用本文
推荐的 `Asia/Shanghai` 容器时区时，于 `08:30` 和 `16:30` 运行，分别对应 UTC
`00:30` 和 `08:30`。公共文件
...
其他站点按青龙容器本地日期保存状态。被跳过的账号不会重复发送通知。

从旧版本升级时，如果 V2EX 当天已被凌晨任务标记完成，请删除一次数据目录下的
`v2ex-state.json`，再运行任务，避免沿用旧版本的本地日期缓存。
任务 cron 仍使用容器当地时间，无需修改整个青龙容器的时区。
