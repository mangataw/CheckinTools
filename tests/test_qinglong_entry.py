...
from qinglong.DefaultTasks import checkin_task_base, checkin_task_setup
...
def test_three_task_metadata_and_shared_base_is_not_a_task():
    ...
    base_source = Path(checkin_task_base.__file__).read_text(encoding="utf-8")
...
def test_single_subscription_prefix_selects_all_qinglong_files():
    ...
    assert filenames == [
        "checkin_task_base.py",
        "checkin_task_fuliba.py",
        "checkin_task_javbus.py",
        "checkin_task_setup.py",
        "checkin_task_v2ex.py",
    ]
    guide = Path("docs/qinglong.md").read_text(encoding="utf-8")
    assert 'ql repo https://github.com/mangataw/CheckinTools.git "checkin_task_"' in guide
    assert "checkin_task_setup.py" in guide


def test_setup_is_not_a_task_and_copies_config_only_once(tmp_path):
    source = Path(checkin_task_setup.__file__).read_text(encoding="utf-8")
    ...
    assert checkin_task_setup.ensure_config(template, destination)
    template.write_text("VALUE='second'\n", encoding="utf-8")
    assert not checkin_task_setup.ensure_config(template, destination)
...
def test_setup_installs_repository_as_editable_package(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(checkin_task_setup.subprocess, "run", runner)
    assert checkin_task_setup.install_project(tmp_path) == 0
...
def test_template_keys_match_runtime_allowlist():
    ...
    assert set(dotenv_values(TEMPLATE)) == checkin_task_base.CONFIG_KEYS


def test_source_layout():
    checkin_task_base._add_source_path()


def test_site_configuration_and_service_dates():
    assert not checkin_task_base._site_is_configured("javbus", {})
    assert checkin_task_base._site_is_configured("javbus", {"JAVBUS_COOKIES": "a"})
    assert checkin_task_base._site_is_configured("fuliba", {"FULIBA_USERNAMES": "b"})
    assert checkin_task_base._site_is_configured("v2ex", {"V2EX_COOKIES": "c"})
    now = datetime(2026, 9, 4, 16, 30, tzinfo=UTC)
    assert checkin_task_base._state_date("v2ex", now) == "2026-09-04"
    assert checkin_task_base._state_date("fuliba", now) == "2026-09-05"
...
def test_run_site_uses_its_own_state_and_lock(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(checkin_task_base, "_run_cli", runner)
    monkeypatch.setattr(checkin_task_base, "_add_source_path", Mock())
    monkeypatch.setattr(checkin_task_base, "_single_instance", lock)
    ...
    assert checkin_task_base.run_site("v2ex") == 0
...
def test_run_site_requires_its_own_account(tmp_path, monkeypatch):
    ...
    assert checkin_task_base.run_site("javbus") == 2
    with pytest.raises(ValueError, match="unknown site"):
        checkin_task_base.run_site("unknown")
...
def test_run_site_validation_and_lock(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(checkin_task_base, "_add_source_path", Mock())
    monkeypatch.setattr(checkin_task_base, "_single_instance", Mock(side_effect=BlockingIOError))
    assert checkin_task_base.run_site("javbus") == 3


def test_source_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(checkin_task_base, "__file__", str(tmp_path / "runtime.py"))
...
def test_cli_does_not_load_dotenv(monkeypatch):
    ...
    assert checkin_task_base._run_cli(["run", "--site", "javbus"], settings) == 0
...
def test_file_is_primary_and_multi_account_syntax(tmp_path, monkeypatch):
    ...
    _, settings = checkin_task_base._load_settings(
...
def test_config_file_rejects_unknown_and_bad_path(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="UNKNOWN_SECRET"):
        checkin_task_base._load_settings({"CHECKIN_QINGLONG_CONFIG": str(path)})
    with pytest.raises(RuntimeError, match="绝对路径"):
        checkin_task_base._load_settings({"CHECKIN_QINGLONG_CONFIG": "relative.env"})
